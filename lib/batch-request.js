    ...
    var batch = function(req, res, next) {
        ...
        var requestPromises = _.reduce(requests, function(promises, r, key) {
            if (!r.dependency || r.dependency === 'none') {
              ...
              _.each(params.defaultHeaders, function(header_v, header_k) {
                if (!(header_k in r.headers)) { // copy defaults if not already exposed
                  r.headers[header_k] = header_v;
                }
              });
              _.each(params.forwardHeaders, function(header_k) {
                if (!(header_k in r.headers) && header_k in req.headers) { // copy forward if not already exposed
                  var forwardValue = req.headers[header_k];
                  r.headers[header_k] = forwardValue;
                }
              });
