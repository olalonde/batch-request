...
var batchRequest = function(params) {
    ...
    params.smartDecode = (typeof params.smartDecode === 'undefined') ? false : params.smartDecode;
        ...
        var requestPromises = _.reduce(requests, function(promises, r, key) {
            if (!r.dependency || r.dependency === 'none') {
                ...
                promises[key] = request(r).spread(function(response, body) {
                    var encoding = 'utf8';
                    if (params.smartDecode) {
                        try {
                            var contentTypeInfo = contentType.parse(response.headers['content-type']);
                            encoding = contentTypeInfo.parameters.charset;
                            if (!Buffer.isEncoding(encoding)) {
                                encoding = mime.charset(contentTypeInfo.type);
                            }
                            if (!Buffer.isEncoding(encoding)) {
                                encoding = 'base64';
                            }
                        } catch (err) {
                            // Could not determine charset
                        }
                    }
                    var decodedBody = body.toString(encoding);
                    return {
